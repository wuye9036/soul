#include <soul/include/syntax_tree/syntax_tree_builder.h>
...
#include <soul/enums/enums_utility.h>
...
using namespace soul::utility;

using soul::common::token_t;

using soul::parser::attribute;
using soul::parser::grammars;
using soul::parser::lexer;
using soul::parser::queuer_attribute;
using soul::parser::selector_attribute;
using soul::parser::sequence_attribute;
using soul::parser::terminal_attribute;
...
#define SOUL_DYNCAST_ATTRIBUTE( type, dest, src ) \
	shared_ptr< type > dest = dynamic_pointer_cast<type>(src);

#define SOUL_SWITCH_RULE( attr ) \
	{ \
...
#define SOUL_CASE_RULE( rule ) else if ( rule_attr_id == g.rule.id() )
#define SOUL_DEFAULT() else
#define SOUL_END_SWITCH_RULE() }
...
shared_ptr<program> syntax_tree_builder::build_prog( shared_ptr< attribute > attr )
{
	...
	SOUL_DYNCAST_ATTRIBUTE(sequence_attribute, typed_attr, attr->child(0));
...
vector< shared_ptr<declaration> > syntax_tree_builder::build_basic_decl( shared_ptr<attribute> attr ){
	...
	SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, semicolon_attr, typed_attr->attr );
	if( semicolon_attr ){
		// ";" only
		return ret;
	}

	SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, not_empty_typed_attr, typed_attr->attr );
	EFLIB_ASSERT_AND_IF( not_empty_typed_attr->attrs.size() == 2, "Basic declaration must not a empty queuer." ){
		return ret;
	}

	SOUL_DYNCAST_ATTRIBUTE( selector_attribute, typed_decl_attr, not_empty_typed_attr->attrs[0] );
	
	SOUL_SWITCH_RULE( typed_decl_attr->attr )
		SOUL_CASE_RULE( vardecl ){
			vector< shared_ptr<variable_declaration> > decls
				= build_vardecl(typed_decl_attr->attr);
			ret.insert( ret.end(), decls.begin(), decls.end() );
		}
		SOUL_CASE_RULE( struct_decl ){
			ret.push_back( build_struct( typed_decl_attr->attr ) );
			return ret;
		}
		SOUL_CASE_RULE( typedef_decl ){
...
shared_ptr<function_full_def> syntax_tree_builder::build_fndecl( shared_ptr<attribute> attr ){
	...
	SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, name_attr, typed_attr->attrs[1] );
	ret->name = name_attr->tok;
	
	SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, paren_params_attr, typed_attr->attrs[2] );
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, optional_params_attr, paren_params_attr->attrs[1] );

	if( !optional_params_attr->attrs.empty() ){
		SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, params_attr, optional_params_attr->attrs[0] );
		...
		// Follows
		SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, follow_params_attr, params_attr->child(1) );
...
shared_ptr<parameter_full> syntax_tree_builder::build_param( shared_ptr<attribute> attr ){
	...
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, optional_ident, typed_attr->attrs[1] );
	build_semantic( typed_attr->attrs[2], ret->semantic, ret->semantic_index );

	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, optional_init, typed_attr->attrs[3] );
	if( !optional_ident->attrs.empty() ){
		SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, ident_attr, optional_ident->attrs[0] );
...
shared_ptr<struct_type> syntax_tree_builder::build_struct( shared_ptr<attribute> attr ){
	...
	SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, typed_attr, attr );
	SOUL_DYNCAST_ATTRIBUTE( selector_attribute, body_attr, typed_attr->attrs[1] );

	SOUL_SWITCH_RULE( body_attr->attr )
		SOUL_CASE_RULE( struct_body ){
			build_struct_body( body_attr->attr, ret );
		}
		SOUL_CASE_RULE( named_struct_body ){
			SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, named_body_attr, body_attr->attr );
			SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, name_attr, named_body_attr->attrs[0] );
			ret->name = name_attr->tok;
			SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, opt_body_attr, named_body_attr->attrs[1] );
...
void syntax_tree_builder::build_struct_body( shared_ptr<attribute> attr, shared_ptr<struct_type> out ){
	assert( out );
	SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, typed_attr, attr );
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, decls_attr, typed_attr->attrs[1] );
...
shared_ptr<expression_list> syntax_tree_builder::build_exprlst( shared_ptr<attribute> attr ){
	...
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, follows, typed_attr->attrs[1] );
	BOOST_FOREACH( shared_ptr<attribute> follow_pair, follows->attrs ){
		SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, typed_follow_pair, follow_pair );
...
{
	SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, tok_attr, attr );
...
shared_ptr<expression> syntax_tree_builder::build_assignexpr( shared_ptr<attribute> attr ){
	...
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, follows, attr->child(1) );
	BOOST_FOREACH( shared_ptr<attribute> follow_pair, follows->attrs ){
		exprs.push_back( 
...
shared_ptr<expression> syntax_tree_builder::dispatch_lcomb_expr( shared_ptr<attribute> attr ){
	...
	SOUL_SWITCH_RULE( attr )
		SOUL_CASE_RULE( lorexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( landexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( borexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( bxorexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( bandexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( eqlexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( relexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( shfexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( addexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( mulexpr ){
			return build_lcomb_expr(attr);
		}
		SOUL_CASE_RULE( castexpr ){
...
shared_ptr<expression> syntax_tree_builder::build_castexpr( shared_ptr<attribute> attr ){
	SOUL_DYNCAST_ATTRIBUTE( selector_attribute, typed_attr, attr );
	SOUL_SWITCH_RULE( typed_attr->attr )
		SOUL_CASE_RULE( unaryexpr ){
			return build_unaryexpr( typed_attr->attr );
		}
		SOUL_CASE_RULE( typecastedexpr ){
...
shared_ptr<expression> syntax_tree_builder::build_unaryexpr( shared_ptr<attribute> attr ){
	SOUL_DYNCAST_ATTRIBUTE( selector_attribute, typed_attr, attr );
	SOUL_SWITCH_RULE( typed_attr->attr )
		SOUL_CASE_RULE( unariedexpr ){
			return build_unariedexpr( typed_attr->attr );
		}
		SOUL_CASE_RULE( postexpr ){
	...
	shared_ptr<expression> expr )
{
	...
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, optional_args, attr );
	...
	shared_ptr<expression> expr )
{
	...
	ret->expr = expr;
	SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, mem_attr, attr->child(1) );
...
shared_ptr<expression> syntax_tree_builder::build_pmexpr( shared_ptr<attribute> attr ){
	SOUL_DYNCAST_ATTRIBUTE( selector_attribute, typed_attr, attr );
	SOUL_SWITCH_RULE( typed_attr->attr )
		SOUL_CASE_RULE( lit_const ){
			SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, const_attr, typed_attr->attr->child(0) );
			...
			SOUL_SWITCH_RULE( const_attr )
				SOUL_CASE_RULE( lit_int ){
					ret->ctype = literal_classifications::integer;
				}
				SOUL_CASE_RULE( lit_float ){
					ret->ctype = literal_classifications::real;
				}
				SOUL_CASE_RULE( lit_bool ){
					ret->ctype = literal_classifications::boolean;
				}
			SOUL_END_SWITCH_RULE();
			return ret;
		}
		SOUL_CASE_RULE( ident ){
			SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, var_attr, typed_attr->attr );
	...
	vector< shared_ptr<variable_declaration> >& decls  )
{
	...
	BOOST_FOREACH( shared_ptr<attribute> follow_attr, follows->attrs ){
		SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, follow_pair, follow_attr );
...
shared_ptr<tynode> syntax_tree_builder::build_unqualedtype( shared_ptr<attribute> attr ){
	...
	SOUL_SWITCH_RULE( typed_attr->attr )
		SOUL_CASE_RULE( ident ){
			...
			SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, ident_attr, typed_attr->attr );
...
shared_ptr<tynode> syntax_tree_builder::build_prequaledtype( shared_ptr<attribute> attr ){
	...
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, quals_attr, typed_attr->attrs[0] );
...
shared_ptr<tynode> syntax_tree_builder::build_postqualedtype( shared_ptr<attribute> attr )
{
	...
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, quals_attr, typed_attr->attrs[1] );
	BOOST_FOREACH( shared_ptr<attribute> qual_attr, quals_attr->attrs ){
...
shared_ptr<initializer> syntax_tree_builder::build_init( shared_ptr<attribute> attr ){
	...
	SOUL_SWITCH_RULE( init_body_attr )
		SOUL_CASE_RULE( assignexpr ){
		...
		SOUL_CASE_RULE( nullable_initlist ){
			EFLIB_ASSERT_UNIMPLEMENTED();
		}
	SOUL_END_SWITCH_RULE();
	...
	SOUL_SWITCH_RULE( typed_attr->attr )
		...
		SOUL_DEFAULT(){
			...
			if( rid != -1 ){
				...
				err += reinterpret_cast<soul::parser::rule*>(rid)->name();
			}
...
shared_ptr<compound_statement> syntax_tree_builder::build_stmt_compound( shared_ptr<attribute> attr ){
	...
	SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, typed_attr, attr );
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, stmts_attr, typed_attr->attrs[1] );
...
shared_ptr<jump_statement> syntax_tree_builder::build_flowctrl( shared_ptr<attribute> attr ){
	...
	SOUL_SWITCH_RULE( stmt_attr )
		SOUL_CASE_RULE( stmt_break ){
			ret->code = jump_mode::_break;
		}
		SOUL_CASE_RULE( stmt_continue ){
			ret->code = jump_mode::_continue;
		}
		SOUL_CASE_RULE( stmt_return ){
			ret->code = jump_mode::_return;
			SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, ret_expr_attr, stmt_attr );
			SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, optional_expr_attr, ret_expr_attr->attrs[1] );
...
shared_ptr<if_statement> syntax_tree_builder::build_stmt_if( shared_ptr<attribute> attr )
{
	...
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, optional_else_stmt, attr->child(5) );
...
shared_ptr<label> syntax_tree_builder::build_label( shared_ptr<attribute> attr )
{
	...
	SOUL_SWITCH_RULE( label_attr )
		SOUL_CASE_RULE( kw_default ){
		...
		SOUL_CASE_RULE( ident ){
			...
			SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, ident_attr, label_attr );
	...
	vector< shared_ptr<variable_declaration> >& declarations )
{
	...
	SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, name_attr, typed_attr->attrs[0] );
	...
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, optional_anno_attr, typed_attr->attrs[3] );
	if( !optional_anno_attr->attrs.empty() ){
		EFLIB_ASSERT_UNIMPLEMENTED();
	}

	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, optional_init_attr, typed_attr->attrs[4] );
...
{
	SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, typed_attr, attr );
	if( !typed_attr->attrs.empty() ){
		SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, sem_attr, typed_attr->attrs[0] );
		SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, sem_name_attr, sem_attr->attrs[1] );
		out_semantic = sem_name_attr->tok;
		SOUL_DYNCAST_ATTRIBUTE( sequence_attribute, optional_semindex_attr, sem_attr->attrs[2] );
		if( !optional_semindex_attr->attrs.empty() ){
			SOUL_DYNCAST_ATTRIBUTE( queuer_attribute, parened_semindex_attr, optional_semindex_attr->attrs[0] );
			SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, index_attr, parened_semindex_attr->attrs[1] );
			out_semantic_index = index_attr->tok;
		}
	}
}

shared_ptr<builtin_type> syntax_tree_builder::get_builtin( boost::shared_ptr<soul::parser::attribute> const& attr )
{
	initialize_bt_cache();
	SOUL_DYNCAST_ATTRIBUTE( terminal_attribute, term_attr, attr );
