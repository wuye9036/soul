...
using soul::common::code_span;
using soul::common::diag_item_committer;
...
fixed_string compiler_code_source::next()
{
	...
	try{
		...
		case preprocess_exception::last_line_not_terminated:
			committer.swap( diags->report( soul::parser::boost_wave_exception_warning ) );
			break;
		case preprocess_exception::ill_formed_directive:
			committer.swap( diags->report( soul::parser::boost_wave_exception_fatal_error ) );
			is_failed = true;
			break;
		default:
			is_failed = true;
...
void compiler_code_source::set_diag_chat( soul::common::diag_chat* diags )
{
	this->diags = diags;
}
...
void report_load_file_failed( void* ctxt, std::string const& name, bool /*is_system*/ )
{
	...
	code_src->diags
		->report( soul::parser::cannot_open_include_file )
